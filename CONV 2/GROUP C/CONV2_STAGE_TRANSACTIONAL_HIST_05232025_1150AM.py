# CONV2_STAGE_TRANSACTIONAL_HIST_05232025_1150AM.py
...
if data_sources.get("ZDM_PREMDETAILS") is not None:
    # Use Contract Account from df_new (which now contains the filtered DFKKOP data)
    contract_accounts = df_new["Cont.Account"].astype(str).str.strip()
    
    # Print samples to debug
    print("Sample df_new Cont.Account values:", contract_accounts.head(5).tolist())
    ...
    dfkkop_ca_set = set(contract_accounts.unique())
    ...
    if len(overlap) == 0:
        ...
        # Approach 2: Format df_new Contract Account with leading zeros
        contract_accounts_formatted = contract_accounts.apply(
            lambda x: x.zfill(12) if pd.notna(x) else x
        )
        
        # Check overlap with formatted values
        formatted_dfkkop_ca_set = set(contract_accounts_formatted.unique())
        ...
        # Apply mapping using the best approach
        if len(formatted_overlap) > 0:
            # Create mapping using formatted keys
            ca_to_locationid = dict(zip(location_lookup["Contract Account Formatted"], location_lookup["LOCATIONID"]))
            df_new["LOCATIONID"] = contract_accounts_formatted.map(ca_to_locationid).fillna("")
            matched_count = (df_new["LOCATIONID"] != "").sum()
            print(f"Merged with formatted keys. LOCATIONID matches: {matched_count}")
        else:
            # Fall back to original fields
            ca_to_locationid = dict(zip(location_lookup["Contract Account"], location_lookup["LOCATIONID"]))
            df_new["LOCATIONID"] = contract_accounts.map(ca_to_locationid).fillna("")
            matched_count = (df_new["LOCATIONID"] != "").sum()
            print(f"Merged with original keys. LOCATIONID matches: {matched_count}")
    else:
    ...
    if (df_new["LOCATIONID"] == "").sum() > 0 and data_sources.get("EVER") is not None:
        print(f"Still missing {(df_new['LOCATIONID'] == '').sum()} LOCATIONID values, trying EVER fallback")
        ...
        print(f"After fallback: Rows with non-empty LOCATIONID: {(df_new['LOCATIONID'] != '').sum()}")
    
    print(f"Final LOCATIONID status: {(df_new['LOCATIONID'] != '').sum()} records have LOCATIONID")
else:
    df_new["LOCATIONID"] = ""
...
# Save to CSV
# --------------------------
output_path = os.path.join(os.path.dirname(list(file_paths.values())[0]), 'STAGE_TRANSACTIONAL_HIST_05232025_1150AM.csv')
