# CONV 2 - STAGE_TRANSACTIONAL_HIST_05282025_1203AM.py

# without TENDERTYPE
# this is not INACTIVEs
# I need to switch from ZMECON from ZDM_PREMDETAILS
...
file_paths = {
    ...
    # ZMECON sources (replacing EVER and ZDM_PREMDETAILS)
...
# --------------------------
# --------------------------
# Extract LOCATIONID through direct merge between DFKKOP and ZMECON
# --------------------------
if data_sources.get("ZMECON") is not None:
    print("\n" + "="*60)
    print("LOCATIONID EXTRACTION - Using ZMECON")
    print("="*60)
    
    # First, let's understand the data formats
    print("\nAnalyzing Partner/BPartner formats for join...")
    
    # Get sample BPartner values from DFKKOP (column B)
    dfkkop_bp_sample = df_new["BPartner"].dropna().head(10)
    print(f"DFKKOP BPartner samples (column B):")
    for bp in dfkkop_bp_sample:
        print(f"  '{bp}' (type: {type(bp).__name__}, len: {len(str(bp))})")
    
    # Get sample Partner values from ZMECON (column A)
    zmecon_df = data_sources["ZMECON"].copy()
    
    # Check column names to find the Partner column (column A)
    print(f"\nZMECON columns: {list(zmecon_df.columns[:10])}")  # Show first 10 columns
    
    # Assuming column A is named "Partner" - adjust if different
    partner_col = zmecon_df.columns[0]  # Column A
    print(f"Using ZMECON column: '{partner_col}' (column A)")
    
    zmecon_partner_sample = zmecon_df[partner_col].dropna().head(10)
    print(f"\nZMECON Partner samples (column A):")
    for partner in zmecon_partner_sample:
        print(f"  '{partner}' (type: {type(partner).__name__}, len: {len(str(partner))})")
    
    # Standardize both sides for matching
    print("\nStandardizing Partner formats for join...")
    
    # For DFKKOP - ensure consistent formatting of BPartner
    df_new["BPartner_Clean"] = df_new["BPartner"].apply(
        lambda x: str(int(float(x))) if pd.notna(x) and str(x).replace('.', '').replace('0', '').strip() else str(x)
    )
    
    # For ZMECON - ensure consistent formatting of Partner
    zmecon_df["Partner_Clean"] = zmecon_df[partner_col].apply(
        lambda x: str(int(float(x))) if pd.notna(x) and str(x).replace('.', '').replace('0', '').strip() else str(x)
    )
    
    # Check for matches
    dfkkop_bp_set = set(df_new["BPartner_Clean"].dropna().unique())
    zmecon_partner_set = set(zmecon_df["Partner_Clean"].dropna().unique())
    overlap = dfkkop_bp_set.intersection(zmecon_partner_set)
    
    print(f"\nMatching statistics:")
    print(f"Unique BPartners in DFKKOP: {len(dfkkop_bp_set)}")
    print(f"Unique Partners in ZMECON: {len(zmecon_partner_set)}")
    print(f"Overlapping Partners: {len(overlap)}")
    
    if len(overlap) == 0:
        print("\nWARNING: No direct matches found. Trying alternative matching...")
        
        # Try matching with leading zeros or other formats
        # Method 1: Remove all leading zeros from both
        df_new["BPartner_NoZeros"] = df_new["BPartner"].apply(
            lambda x: str(x).lstrip('0') if pd.notna(x) and str(x).strip() else ""
        )
        zmecon_df["Partner_NoZeros"] = zmecon_df[partner_col].apply(
            lambda x: str(x).lstrip('0') if pd.notna(x) and str(x).strip() else ""
        )
        
        overlap_nozeros = set(df_new["BPartner_NoZeros"].dropna().unique()).intersection(
            set(zmecon_df["Partner_NoZeros"].unique())
        )
        
        print(f"Overlap without leading zeros: {len(overlap_nozeros)}")
        
        if len(overlap_nozeros) > 0:
            bp_field_dfkkop = "BPartner_NoZeros"
            partner_field_zmecon = "Partner_NoZeros"
            print("Using no-zeros format for matching")
        else:
            bp_field_dfkkop = "BPartner_Clean"
            partner_field_zmecon = "Partner_Clean"
            print("Using cleaned format for matching")
    else:
        bp_field_dfkkop = "BPartner_Clean"
        partner_field_zmecon = "Partner_Clean"
        print("Using cleaned format for matching")
    
    # Create the mapping from Partner to Premise (LOCATIONID)
    print(f"\nCreating Partner to LOCATIONID mapping from ZMECON...")
    
    # Find the Premise column in ZMECON
    premise_col = zmecon_df.columns[25]  # Column Z
    
    if premise_col is None:
        print("WARNING: Could not find 'Premise' column in ZMECON. Checking column names...")
        print(f"ZMECON columns: {list(zmecon_df.columns)}")
        # You may need to adjust this based on actual column name
        premise_col = "Premise"  # Adjust if needed
    
    partner_to_locationid = {}
    for _, row in zmecon_df.iterrows():
        partner = row[partner_field_zmecon]
        premise = str(row[premise_col]).strip() if premise_col in zmecon_df.columns else ""
        if pd.notna(partner) and pd.notna(premise) and partner and premise:
            partner_to_locationid[partner] = premise
    
    print(f"Created mapping with {len(partner_to_locationid)} entries")
    
    # Apply the mapping
    df_new["LOCATIONID"] = df_new[bp_field_dfkkop].map(partner_to_locationid).fillna("")
    ...
    # Final diagnostic - show some unmatched BPartners
    if unmatched_count > 0:
        print("\nSample unmatched BPartners:")
        unmatched_sample = df_new[df_new["LOCATIONID"] == ""][bp_field_dfkkop].dropna().head(10)
        for bp in unmatched_sample:
            print(f"  '{bp}'")
    
    # Clean up temporary columns
    temp_cols = ["BPartner_Clean", "BPartner_NoZeros"]
    ...
    df_new["LOCATIONID"] = ""
    print("ZMECON not available, LOCATIONID set to empty")
# --------------------------
...
# Check ZMECON for MRU=METRNP01 to set APPLICATION=2
if data_sources.get("ZMECON") is not None:
    ...
    if mru_col:
        ...
        if metrnp01_count > 0:
            ...
            if premise_col:
                ...
                for premise in metrnp01_records[premise_col].astype(str):
        ...
        print("WARNING: Could not find MRU column in ZMECON")
...
# Save to CSV
# --------------------------
output_path = os.path.join(os.path.dirname(list(file_paths.values())[0]), 'STAGE_TRANSACTIONAL_HIST_05282025_1203PM.csv')
