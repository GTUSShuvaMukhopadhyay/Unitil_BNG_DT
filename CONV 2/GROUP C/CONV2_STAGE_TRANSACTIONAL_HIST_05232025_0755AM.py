# CONV2_STAGE_TRANSACTIONAL_HIST_05232025_0755AM.py

# BHAVYA & SHUVA - we will use the code from CONV1 in GITHUB as a base and add the changes needed for CONV2
# fields we need to update are:
# 1. TRANSACTIONTYPE
...
# Define the 19 valid combinations - MTrans as string, STrans as float
valid_combinations = {
    ("0015", 10.0), ("0015", 20.0), ("0015", 21.0), ("0015", 30.0),
    ("0015", 40.0), ("0015", 70.0), ("0015", 230.0), ("0015", 231.0),
    ("0015", 300.0), ("0015", 370.0), ("0015", 371.0), ("0025", 10.0),
    ("0070", 10.0), ("0080", 5.0), ("0080", 10.0), ("0100", 2.0),
    ("0200", 2.0), ("0620", 10.0), ("0630", 10.0)
}

# Store original count
original_count = len(data_sources["DFKKOP"])

# Apply filter - MTrans is already string, STrans is float
data_sources["DFKKOP"] = data_sources["DFKKOP"][
    data_sources["DFKKOP"].apply(
        lambda row: (str(row['MTrans']).strip(), float(row['STrans']) if pd.notna(row['STrans']) else None) in valid_combinations, 
        axis=1
    )
]
...
if data_sources.get("DFKKOP") is not None:
    ...
    print(f"Extracted TRANSACTIONAMOUNT")
...
if data_sources.get("ZDM_PREMDETAILS") is not None and data_sources.get("DFKKOP") is not None:
    ...
    if metrnp01_count > 0:
        ...
        # Make sure LOCATIONID is a string column
        df_new["LOCATIONID"] = df_new["LOCATIONID"].astype(str)
        
        # Apply the mapping using vectorized operations
        updated_count = 0
        for premise in location_to_application:
            mask = df_new["LOCATIONID"].str.replace('"', '') == premise
...
#hard-coded values
# --------------------------
# Extract TENDERTYPE from DFKKZP
# --------------------------
if data_sources.get("DFKKZP") is not None and data_sources.get("ZDM_PREMDETAILS") is not None:
    print("\nSetting TENDERTYPE values from DFKKZP...")
    
    # Define DT to TENDERTYPE mapping
    dt_mapping = {
        'CA': '70',
        'CK': '71', 
        'CR': '81',
    ...
    # Step 1: Create lookup from Contract Account to Business Partner using ZDM_PREMDETAILS
    zdm_df = data_sources["ZDM_PREMDETAILS"].copy()
    # Standardize Contract Account format - remove .0 decimal
    ...
    zdm_df["Business Partener"] = zdm_df["Business Partener"].apply(
        lambda x: str(int(float(x))) if pd.notna(x) and str(x).replace('.', '').replace('0', '').strip() else str(x)
    ).str.strip()
    
    # Create Contract Account to Business Partner mapping
    ca_to_bp = {}
    for ca, bp in zip(zdm_df["Contract Account"], zdm_df["Business Partener"]):
        if pd.notna(ca) and pd.notna(bp) and ca and bp:
            ca_to_bp[ca] = bp
    print(f"Created Contract Account to Business Partener mapping with {len(ca_to_bp)} entries")
    
    # Step 2: Process DFKKZP records
    dfkkzp_df = data_sources["DFKKZP"].copy()
    # Standardize Selection Value 1 format
    dfkkzp_df["Selection Value 1"] = dfkkzp_df["Selection Value 1"].astype(str).str.strip()
    
    # Create Business Partner to TENDERTYPE mapping
    bp_to_tendertype = {}
    processed_count = 0
    r_override_count = 0
    
    for idx, row in dfkkzp_df.iterrows():
        ca = str(row["Selection Value 1"]).strip()
        dt = str(row["DT"]).strip() if pd.notna(row["DT"]) else ""
        r_val = str(row["R"]).strip() if pd.notna(row["R"]) else ""
        
        # Get Business Partner from Contract Account
        if ca in ca_to_bp:
            bp = ca_to_bp[ca]
            
            # Determine TENDERTYPE based on rules
            if r_val in ['1', '2', '1.0', '2.0']:  # Handle both with and without decimals
                tendertype = '94'
                r_override_count += 1
            elif dt in dt_mapping:
                tendertype = dt_mapping[dt]
            else:
                continue  # Skip if no valid mapping
            
            # Store the mapping (last one wins if duplicate BP)
            bp_to_tendertype[bp] = tendertype
            processed_count += 1
    
    print(f"Processed {processed_count} DFKKZP records")
    print(f"R-value overrides (TENDERTYPE=94): {r_override_count}")
    print(f"Created Business Partner to TENDERTYPE mapping with {len(bp_to_tendertype)} entries")
    
    # Step 3: Apply TENDERTYPE to df_new based on CUSTOMERID (which is BPartner)
    df_new["TENDERTYPE"] = ""  # Initialize
    matched_count = 0
    
    for idx in range(len(df_new)):
        customerid = str(df_new.at[idx, "CUSTOMERID"]).replace('"', '').strip()
        if customerid in bp_to_tendertype:
            df_new.at[idx, "TENDERTYPE"] = bp_to_tendertype[customerid]
            matched_count += 1
    
    print(f"Updated {matched_count} records with TENDERTYPE values")
    
    # Show distribution of TENDERTYPE values
    if matched_count > 0:
        tendertype_counts = df_new[df_new["TENDERTYPE"] != ""]["TENDERTYPE"].value_counts()
        print("TENDERTYPE distribution:")
        for tt, count in tendertype_counts.items():
            print(f"  {tt}: {count} records")
else:
    # Default to empty if files not available
    df_new["TENDERTYPE"] = ""
    print("DFKKZP or ZDM_PREMDETAILS not available, TENDERTYPE set to empty")
...
if data_sources.get("DFKKZP") is not None and data_sources.get("ZDM_PREMDETAILS") is not None:
    ...
    zdm_df = data_sources["ZDM_PREMDETAILS"].copy()
    zdm_df["Contract Account"] = zdm_df["Contract Account"].astype(str).str.strip()
    zdm_df["Business Partener"] = zdm_df["Business Partener"].astype(str).str.strip()
    
    # Create Contract Account to Business Partner mapping
    ca_to_bp = {}
    for ca, bp in zip(zdm_df["Contract Account"], zdm_df["Business Partener"]):
        if pd.notna(ca) and pd.notna(bp):
            ca_to_bp[ca] = bp
    print(f"Created Contract Account to Business Partener mapping with {len(ca_to_bp)} entries")
...
# Save to CSV
# --------------------------
output_path = os.path.join(os.path.dirname(list(file_paths.values())[0]), 'STAGE_TRANSACTIONAL_HIST_05232025_0755AM.csv')
