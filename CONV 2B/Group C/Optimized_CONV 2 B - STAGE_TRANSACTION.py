...
if dfkkop_keys:
    ...
    if dfkkop_dfs:
        ...
        mtrans_mask = data_sources["DFKKOP"]["MTrans"].notna() & data_sources["DFKKOP"]["MTrans"].apply(lambda x: isinstance(x, (int, float)))
        strans_mask = data_sources["DFKKOP"]["STrans"].notna() & data_sources["DFKKOP"]["STrans"].apply(lambda x: isinstance(x, (int, float)))
...
if len(debug_transactional) > 0:
    ...
    for idx, row in sample_records.iterrows():
        print(f"   Row {row['ROW_INDEX']}: Date: {row['DOC_DATE']}, Raw: {row['RAW_REFERENCE']}, Processed: {row['PROCESSED_BILLORINVOICE']}, Amount: {row['AMOUNT']}")
...
for col in df_new.columns:
    if col not in ['Doc. Date', 'Pstng Date', 'Due', 'Reference', 'BPartner', 'Cont.Account', 'Amount', 'MTrans', 'STrans']:
