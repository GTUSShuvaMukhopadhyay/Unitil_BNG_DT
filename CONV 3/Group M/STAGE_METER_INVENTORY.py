...
CHECKLIST = [
    ...
    "✅ Ensure all dates are in 'YYYYMMDD' format.",
]
...
# File paths
file_path1 = r"C:\Users\US82783\OneDrive - Grant Thornton Advisors LLC\Desktop\python\CONV 3\DATA SOURCES\ZINS.XLSX"
file_path2 = r"C:\Users\US82783\Downloads\BNG Gas Meter Attributes Cleanup Conv_3 08_07_25.xlsx"
config_path = r"C:\Users\US82783\OneDrive - Grant Thornton Advisors LLC\Desktop\python\CONV 3\DATA SOURCES\Configuration 13.xlsx"
file_path3 = r"C:\Users\US82783\Downloads\Meter Details Report.xlsx"
print("🔄 Loading input files...")

# Read the Excel file and load the specific sheets
bng_gas_meter_df = pd.read_excel(file_path2, sheet_name='ZINS and Meter Details Table', engine='openpyxl')

zins_raw = pd.read_excel(file_path1, sheet_name='Sheet1', engine='openpyxl')
'''
# Filter ZINS (Status & Current Address blank)
zins_filtered = zins_raw[
    (zins_raw.iloc[:, 1].isna() | (zins_raw.iloc[:, 1].astype(str).str.strip() == '')) &
    (zins_raw.iloc[:, 6].isna() | (zins_raw.iloc[:, 6].astype(str).str.strip() == ''))
]

# Standardize ZINS to have a "MeterNumber" column
zins_std = pd.DataFrame({
    "MeterNumber": zins_filtered.iloc[:, 0].astype(str).str.strip()
})'''

zins_std = pd.DataFrame({
    "MeterNumber": zins_raw.iloc[:, 0].astype(str).str.strip()
})

# Read Meter Details Report
meter_details = pd.read_excel(file_path3, sheet_name='Sheet1', engine='openpyxl')

# Standardize Meter Details Report → use Serial Number as meter number
extra_std = pd.DataFrame({
    "MeterNumber": meter_details.iloc[:, 1].astype(str).str.strip()
})

# Combine both sources
combined_meters = pd.concat([zins_std, extra_std], ignore_index=True)

print(f"✅ ZINS + Meter Details Report combined. Total meters: {len(combined_meters)}")

# Replace zins_df with the combined set
zins_df = combined_meters
...
def fetch_podate(meter_number):
    ...
    bng_gas_meter_clean = bng_gas_meter_df.iloc[:, 0].astype(str).str.strip()
...
# Save to CSV
output_path = r"C:\Users\US82783\OneDrive - Grant Thornton Advisors LLC\Desktop\python\CONV 3\Extracts\STAGE_METER_INVENTORY_without filter on ZINS.csv"
