...
# FIXED: Define robust function for BILLORINVOICENUMBER extraction (same as DFKKOP)
...
for idx, row in base_df.iterrows():
    ...
    common_data = {
        ...
        'BILLORINVOICENUMBER': clean_billorinvoice(data_sources["ZMECON"].iloc[idx, 3]),        # Fixed field values
        'TAXYEAR': "",
        'APPLICATION': 5,
        'BILLTYPE': 0,
        'TENDERTYPE': 0,
    ...
    service_row.update({
        'TRANSACTIONTYPE': 99,
        'TRANSACTIONAMOUNT': round(data_sources["ZMECON"].iloc[idx, 8], 2) if pd.notna(data_sources["ZMECON"].iloc[idx, 8]) else 0,
...
# )
    # SM Update for NaN Values
    # Columns J, L, N, P: "Energy Charge", "Past Gas Adj
    energy_amount = (
        (data_sources["ZMECON"].iloc[idx, 9] if pd.notna(data_sources["ZMECON"].iloc[idx, 9]) else 0) +   # Column J: "Energy Charge"
        (data_sources["ZMECON"].iloc[idx, 11] if pd.notna(data_sources["ZMECON"].iloc[idx, 11]) else 0) +  # Column L: "Past Gas Adj.Charges"
        (data_sources["ZMECON"].iloc[idx, 13] if pd.notna(data_sources["ZMECON"].iloc[idx, 13]) else 0) +  # Column N: "Efficiency mine fund"
        (data_sources["ZMECON"].iloc[idx, 15] if pd.notna(data_sources["ZMECON"].iloc[idx, 15]) else 0)    # Column P: "Transp.Charge"
    )
    energy_row = common_data.copy()
    energy_row.update({
        'TRANSACTIONTYPE': 2,
        'TRANSACTIONAMOUNT': round(energy_amount, 2),
    ...
    tax_row.update({
        'TRANSACTIONTYPE': 99,
        'TRANSACTIONAMOUNT': round(data_sources["ZMECON"].iloc[idx, 17], 2) if pd.notna(data_sources["ZMECON"].iloc[idx, 17]) else 0,  # Column R: "Sale tax (Type 99)"
...
# Save to CSV
# --------------------------
output_path = r"C:\Users\GTUSER1\Documents\CONV 3\output\Group C\STAGE_TRANSACTIONAL_HIST_ZMECON.csv"
