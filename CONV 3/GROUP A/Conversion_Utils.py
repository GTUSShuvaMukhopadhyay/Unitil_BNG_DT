...
# Version: 1.0
# pathname: CUsersGTUSER1DocumentsCONV 3
# -----------------------------------------------------------------------------
# Notes:
...
try:
    ...
    Source_Schemas = None
    print("Warning: Source_Schemas module not found. Using default string types for all columns.")


# Constants - Updated for CONV 3 directory structure
source_directory = r"c:\Users\GTUSER1\Documents\CONV 3\\"
cache_directory = r"c:\Users\GTUSER1\Documents\CONV 3\parquet\\"
output_directory = r"c:\Users\GTUSER1\Documents\CONV 3\output\\"

# Updated file_paths dictionary based on the screenshot
file_paths = {
    # Source files from CONV 3 directory
    ...
    "zmecon": source_directory + r"ZMECON 08012019 to 08012025.xlsx",
    ...
    # Output files (keeping your existing structure)
...
def read_filepath( file_name, file_path, sheet_name, columns):
    ...
    if Source_Schemas and hasattr(Source_Schemas, file_name + "_schema"):
...
# Read cache file
    ...
    # If specific columns are requested, filter the DataFrame - FIXED INDENTATION
    if columns:
...
def write_csv(df, file_name):
    ...
    Headers are unquoted, string data is quoted, TRAILER row is unquoted.
    ...
    # Check if TRAILER row already exists
    has_trailer = False
    if not df_copy.empty:
        # Check if the last row's first column contains 'TRAILER'
        last_row_first_col = str(df_copy.iloc[-1, 0]).strip().upper()
        if last_row_first_col == 'TRAILER':
            has_trailer = True
            log_info("TRAILER row already exists, skipping addition of new TRAILER row.")
    
    # Clean the data - remove existing quotes and clean strings
    for col in df_copy.columns:
        df_copy[col] = df_copy[col].apply(lambda x: clean_csv_value(x))

    # Replace NA with empty string
    df_copy = df_copy.fillna('')

    # Add a trailer row only if one doesn't exist
    if not has_trailer:
        trailer_row = pd.DataFrame([['TRAILER'] + [''] * (len(df_copy.columns) - 1)], columns=df_copy.columns)
        df_copy = pd.concat([df_copy, trailer_row], ignore_index=True)
        log_info("Added TRAILER row to the end of the DataFrame.")

    # Write to CSV manually to control quoting exactly
    ...
    with open(output_path, 'w', newline='', encoding='utf-8') as f:
        ...
        for index, row in df_copy.iterrows():
            ...
            for i, val in enumerate(row):
                ...
                # Check if this is the TRAILER row
                is_trailer_row = (str(row.iloc[0]).strip().upper() == 'TRAILER')
                
                if is_trailer_row:
                    # TRAILER row - no quotes
                    row_values.append(val_str)
                else:
                    # Regular data row - quote non-empty strings
                    if val_str != '':
    ...
    log_info(f"CSV file saved at {output_path} with {len(df_copy)} rows.")

def clean_csv_value(value, max_length=None):
    """
    Cleans a value for CSV output by removing existing quotes and whitespace.
    """
    if pd.isna(value):
        return ''
    ...
    # Remove existing quotes (both single and multiple)
    value = re.sub(r'^"+|"+$', '', value)  # Remove quotes from start/end
    value = re.sub(r'"{2,}', '"', value)   # Replace multiple quotes with single
