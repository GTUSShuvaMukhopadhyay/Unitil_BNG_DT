...
file_path = r"c:\Users\GTUSER1\Documents\CONV 3\ZCAMPAIGN\ZCAMPAIGN - Communication.XLSX"
...
df_new['PHONENUMBER'] = (
    df.iloc[:, 8]
    .fillna('')
    .apply(lambda x: str(x).split('.')[0])         # Remove decimal if present
    .apply(lambda x: ''.join(filter(str.isdigit, x)))  # Remove any non-digits just in case
    .apply(lambda x: x[-10:] if len(x) > 10 else x)    # Keep only last 10 digits
)
...
output_csv = r"c:\Users\GTUSER1\Documents\CONV 3\output\Group A\STAGE_PHONE.csv"
# os.path.join(os.path.dirname(file_path), 'STAGE_PHONE.csv')
