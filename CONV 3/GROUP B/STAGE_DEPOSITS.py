# CONV 2 B - STAGE_DEPOSITS

# spoke with Gary - we will use the 6.14 data for this but need to update script for newest data for ZMECON.
# had to use  new data for FPD2,ZDMPREM
...
# Define file paths
file_paths = {
    "FPD2": r"c:\Users\GTUSER1\Documents\CONV 3\FPD2 - Full Report - 0802.XLSX",
    ...
    "DFKKOP1": r"c:\Users\GTUSER1\Documents\CONV 3\DFKKOP\DFKKOP 01012019 to 12312019.XLSX",
    "DFKKOP2": r"c:\Users\GTUSER1\Documents\CONV 3\DFKKOP\DFKKOP 01012020 to 12312020.XLSX",
    "DFKKOP3": r"c:\Users\GTUSER1\Documents\CONV 3\DFKKOP\DFKKOP 01012021 to 12312021.XLSX",
    "DFKKOP4": r"c:\Users\GTUSER1\Documents\CONV 3\DFKKOP\DFKKOP 01012022 to 12312022.XLSX",
    "DFKKOP5": r"c:\Users\GTUSER1\Documents\CONV 3\DFKKOP\DFKKOP 01012023 to 12312023.XLSX",
    "DFKKOP6": r"c:\Users\GTUSER1\Documents\CONV 3\DFKKOP\DFKKOP 01012024 to 12312024.XLSX",
    "DFKKOP7": r"c:\Users\GTUSER1\Documents\CONV 3\DFKKOP\DFKKOP 01012025 to 12312025.XLSX",
}
...
def load_excel(name, path):
    try:
        ...
        # For ZMECON files, we only need columns 0, 22, and 24
            ...
            temp_df = pd.read_excel(path, sheet_name="ZMECON", engine="openpyxl", nrows=5)
            if len(temp_df.columns) >= 25:  # Make sure we have enough columns
                return pd.read_excel(path, sheet_name="ZMECON", engine="openpyxl", 
                                   usecols=[0, 22, 24], dtype=str)
            else:
                # Fallback to loading essential columns if structure differs
                return pd.read_excel(path, sheet_name="ZMECON", engine="openpyxl", dtype=str)

        # For FPD2, we need columns 0, 4, 8, 10
        elif name == "FPD2":
            return pd.read_excel(path, sheet_name="Sheet1", engine="openpyxl", 
                               usecols=[0, 4, 8, 10], dtype=str)
            # Apply filter for "Description of Security Deposit Status" (column K, which is index 10)
            # Note: After loading with usecols=[0, 4, 8, 10], column K becomes index 3 in the resulting dataframe
            if len(df.columns) >= 4:  # Make sure we have the expected columns
                status_column = df.columns[3]  # Column K is now at index 3
                # Filter for only "Paid" or "Partially Returned"
                df = df[df[status_column].isin(["Paid", "Partially Returned"])]
                print(f"FPD2 filtered to {len(df)} rows with 'Paid' or 'Partially Returned' status")
...
if zmecon1 is not None:
    data_sources["ZMECON"] = zmecon1
...
if dfkkop_dfs:
    ...
    if len(data_sources["DFKKOPA"].columns) >= 4:  # Make sure we have expected columns
        ...
        # Pre-process data to avoid repeated operations
        data_sources["DFKKOP"] = data_sources["DFKKOP"].copy()
    ...
    if data_sources["FPD2"] is not None:
        try:
            ...
            if "CUSTOMERID" in data_sources["FPD2"].columns:
                ...
                # ).str.slice(0, 15)
                # SM changed cuz of FLOAT NaN to Integer error 8_7_25
                df_new["CUSTOMERID"] = data_sources["FPD2"]["CUSTOMERID"].apply(
            ...
            def calculate_refund_months(customerid):
                if customerid in zmecon_lookup:
                    status = zmecon_lookup[customerid]["status"] # change to rate1 column y
            ...
            df_new["DEPOSITKIND"] = "1"
            df_new["DEPOSITSTATUS"] = "0"  # Default value
            ...
            df_new["DEPOSITINTERESTCALCDATE"] = " "
            ...
            # Save to CSV
            # output_path = os.path.join(os.path.dirname(list(file_paths.values())[0]), '1511pm_STAGE_DEPOSITS.csv')
            output_path = r"C:\Users\GTUSER1\Documents\CONV 3\output\Group B\STAGE_DEPOSITS.csv"
